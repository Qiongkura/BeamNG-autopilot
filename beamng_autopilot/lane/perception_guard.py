...
def perception_lateral_guard(
    ...
    * edge guard: when either perceived road edge is closer than
    ...
    if n < 8 or res <= 0.0:
        return 0.0
    # Rows: row 0 is the most forward (+x); r = (extent - x) / res.
    ...
    # The road directly in front (2-4 m) is often below the camera's
    # road mask (bonnet blind spot), so the guard reads a WIDE band
    # 2-12 m ahead.  Near (2-6 m) and far (7-12 m) sub-bands are only
    # used for the perceived-corner test.
    r0w, r1w = _rows(near_m, look_ahead_m)
    wide = _extent_of(r0w, r1w, min_cells)
    if wide is None:
        return 0.0
    y_left, y_right = wide[1], wide[0]
    center = 0.5 * (y_left + y_right)
    ...
    near_ext = _extent_of(r0n, r1n, max(6, min_cells // 2))
    far_ext = _extent_of(r0f, r1f, max(6, min_cells // 2))
    if near_ext is not None and far_ext is not None:
        near_center = 0.5 * (near_ext[0] + near_ext[1])
        far_center = 0.5 * (far_ext[0] + far_ext[1])
        if abs(far_center - near_center) > corner_shift_m:
            return 0.0
