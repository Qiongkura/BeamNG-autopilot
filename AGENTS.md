...
## 驾驶约束（用户硬性要求，优先级高于便利性）

1. **右侧边界的信任顺序：标线 >> 路面边界 = 护墙 = 围栏。**
   有漆线时必须用漆线；没有漆线时才允许用道路外沿（铺装路缘 / 护栏 / 围栏）。
   这一档的每个来源都必须是**真的被感知到**的，不允许用"疑似"或推断值凑。
2. **有铺装路面时，土肩不得算作道路。** 车必须待在铺装面上；路面掩码里的
   非铺装地面要被剔除（见 `vision.segmentation.strip_soil_from_road`）。
3. **只有当路线本身就是土路（没有铺装）时，土才算路面。** 这两种情况由
   "掩码里铺装占比"区分，不要用单一全局规则。
4. **没有标线时要让车保持在道路右侧**（右行）。若这一档的边界来源不可信
   （既无标线、又无护栏/围栏、路面边界也测不准），合法行为是 fail-closed
   停车，**不是**拿一个猜测的边界去开。
5. **strict FSD 绝不倒车、绝不压线。** `ReverseManeuver` 在 strict 下必须
   `enabled=False`；车身已经压线时只允许"收敛回车道"的缓行（见
   `body_lane_cross_recovery`），不允许为了找路倒车或横向漂移。
6. **实车演示必须带导航路线**（`--goal X Y`）。没有路线时地图路缘守卫失效、
   速度被压到 1 m/s，车会沿感知车道漂出去——这不是"掉线"，是配置错误。

## 提交纪律（用户多次强调）

一个提交 = **一个模块的完整改动（含它的测试）**：

- 模块边界：`beamng_autopilot/<子包或顶层模块>`、`scripts/`、`tests/`、`docs/`、
  `AGENTS.md` 各自独立。
- **不要把两个模块混进一个提交**（例如 `safety_monitor.py` + `fsd_drive.py`）；
  需要一起改时，先确认它们是否属于同一个模块，不是就拆开。
- **也不要按文件/微步骤拆成一堆细碎提交**：同一模块的同一次改动写清动机、
  证据与验证结果，一次提交完。
- 一次改动天然跨模块时（例如"加辅助函数 → 判定 → 结论"），拆成有序的多个
  提交，但**每个提交必须自身可编译、`pytest tests/` 可通过**：先提交纯新增
  （辅助函数 + 单测），再提交接线，最后提交行为变更。做不到就说明改动本身
  应该重新设计。
- 推送前自查：`.venv\Scripts\python.exe scripts\check_commit_scope.py`
  （检查 `@{u}..HEAD`，也可 `--last N`）；发现混模块就先重做再推。
...
> 原假设 → 是否真正激活 → 可复现输入 → 实测结果 → 支持/推翻的结论 → 未知与下一步

并且必须附下面八项，**不能只写"全绿"**：

1. commit / config / run 清单，及无控制权冲突的证据；
2. 测量覆盖率、缺列情况、UNKNOWN 数量；
3. 强制刷新触发与实际新源结果被消费的证据；
4. 几何正反例与最终控制链证据；
5. 路面门真阳性、漏检与误停；
6. 性能分解与 deadline 违反情况；
7. 碰撞、压线、出铺装，以及按原因分类的停车时长；
8. 通过项、失败项、尚未测试项。

完整原文与出处见 `docs/REPORT_FORMAT.md`。本轮最小成功标准不是跑得更远，
而是：能解释数据如何进入控制；能用确定性例子区分"有空隙"和"可安全执行"；
能证明硬约束最终生效；能说清性能瓶颈与横向感知仍缺什么。
