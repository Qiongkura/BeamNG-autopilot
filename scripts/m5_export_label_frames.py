...
------------------------------------------------
窗口按键 → 类别 → 保存的标签值
  1 = line   → 标签值 2   漆画线本体（白/黄实虚线）
  2 = road   → 标签值 1   可行驶的**铺装**路面（沥青 / 水泥）
  3 = erase  → 标签值 0   背景；**没涂的像素也是 0**

与老流程的区别（务必看清，别按老习惯理解 3）
* 以前的人工包是「只标线」模式（训练时 `--line-only-runs`）：那时 3 擦除
  = **忽略(255)**，只有线像素参与训练，擦掉的像素不教任何东西。
* 本次的目标是教模型「土肩不是道路」，所以土肩必须成为**背景监督(0)**，
  整个包用**完整标签模式**训练（`--runs`，不加 `--line-only-runs`）。
  不加时 3 擦除 = 背景(0)，这是本次要的语义。

要点
* 铺装路两侧的土肩 / 砂石带 / 草地 → 涂 3（=背景）。
* 如果整帧**没有铺装**（纯土路/砂石路）→ 那条土路本身涂 2（=road）。
* 路缘石属于路面（2）；它外侧的地面是 3。
* 没涂的像素算背景(0)，所以天空/植被/车体不涂也对，但**别把该涂的路漏掉**。

工具
* 左键涂；右键 / `f` 油漆桶；`1/2/3` 切换类别；`u` 撤销；`c` 清空；
  `s` 保存并下一张；`q` 退出。
"""
