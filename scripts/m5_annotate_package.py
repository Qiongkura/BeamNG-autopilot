...
KEY_HELP = """\
界面顶端有一条工具栏，**所有操作都能用鼠标点**（按钮上印着等价的键）：

| 顶端按钮 | 等价的键 | 作用 |
| --- | --- | --- |
| Lane / Road / Erase | `1` / `2` / `3` | 当前类别 = 标线 / 路面 / 背景（擦除） |
| Occl / Blur / Undec | `4` / `5` / `6` | 标记"这里为什么画不了"：遮挡 / 模糊 / 无法判断 |
| Pen | `p` | 画笔：左键拖动，画当前类别 |
| Bucket | `f`（或右键） | 油漆桶（填连通区域：先画闭合轮廓，再点内部） |
| Straight | `l` | 直线：拖动 A→B，或点两下（A 再 B）—— 画出来是**笔直**的 |
| Curve | `v` | 平滑曲线：拖动 = 自由手绘（自动去抖 + 平滑）；或点若干控制点后按 `ENTER`，曲线穿过每个点 |
| Undo | `u` | 撤销：画曲线时退掉最后一个控制点，否则退掉上一笔 |
| Clear / Zoom / Prev / Save+Next | `c` / `z` / `a`(或 ←) / `s` | 清空整帧 / 2×-1× / 上一帧 / 保存并下一帧 |
| 拖动条 | — | 笔刷大小 1–40 |
