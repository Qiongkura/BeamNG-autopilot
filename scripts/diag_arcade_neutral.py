# Verify the exact handover tail: realistic, stopped, forward gear engaged,
# then shift to N + parking brake, then switch to arcade.  Does the car stay
# truly parked, or does arcade D creep forward on its own?
...
def main() -> None:
    ...
    try:
        ...
        # Reproduce the handover tail: realistic -> probe D -> stop -> N.
        fwd = forward_gear_input(conn)
        print(f"[diag] forward gear input = {fwd}")
        snap(conn, "realistic D stopped pb=1")

        # Shift to neutral while stopped with the parking brake on.
        conn.control(throttle=0.0, brake=0.0, steering=0.0,
                     parkingbrake=1.0, gear=0)
        conn.step(10)
        snap(conn, "realistic N stopped pb=1")

        # Switch to arcade and watch 4 s: does it stay put?
        lua(conn, 'controller.mainController.setGearboxMode("arcade")')
        conn.step(10)
        snap(conn, "arcade just after switch")
        worst = 0.0
        best = 0.0
        for i in range(40):
            time.sleep(0.1)
        ...
        print(f"[diag] RESULT roll range [{worst:+.2f}, {best:+.2f}] m/s")
    finally:
        try:
            conn.control(throttle=0.0, brake=0.0, steering=0.0,
                         parkingbrake=1.0)
