...
## 21. corridor 候选实车否决 + 两处口径修正，影响 P1 判据（2026-09-12 凌晨，另一会话）

### 21a. 「可行驶右边缘 + 半车道宽」实车否决，机制已量化

为绕开成对率做了一个免配对候选：BEV 自由走廊右边缘 + 半车道宽（`lane/reference.py::
bev_corridor_lane_center`，旗标 `--corridor-lane`，**默认关**）。离线（24 集 4951 帧）
曾给出 100% 覆盖 / 95.1% in-lane，**实车单臂直接否决**（`town_1789142315`，town 场景，
其余配置与 09-11 深夜 A/B 的 hand 臂完全相同）：

| 量 | 值 |
| --- | --- |
| 真值右车道线（GT annotation，近场 3–12 m 中位） | **−1.43 m** |
| 同帧可行驶栅格右边缘 | **−3.75 m**（越过车道线向右溢出 ~1.9 m 路肩） |
| 候选参考相对真值车道中心的误差 | **p50 −1.24 m，52.7% 帧误差 >1.2 m** |
| 该轮倒车 / stall | **40 帧 / 155**（hand 臂为 4 / 113） |
| 车实际位置（GT 判） | 220 帧 100% \|偏心\|<1.2 m（车没跟着错误参考走：爬行+纠偏层拉回） |

**机制**：可行驶掩码的右边缘是**道路右边缘**（含路肩），不是本车道右边界；
"双车道"与"双车道+右路肩"总带宽相同（都 ~8.5 m），总带宽门控在原理上就拦不住。
**该假设运行时不可验证 → 结构性不可靠，任何离线 in-lane 数字不构成上线依据**。
代码/docstring/两个入口的 help 已全部标注 REFUTED；`corridor_fallback` 必须保持 False，
除非重设计为"边界锚定漆画线"并重新过实车安全门（line_lat 居中、0 压线/出路面/倒车）。

### 21b. `m5_seg_task_eval.py` 的 in-lane 目标从 −1.75 修正为 0（**影响 P1 门限**）

三路证据一致表明**车在车道内时"配对中心"读数 ≈ 0**（左 = +）：
1. 城镇 GT 漆画左右线中点 ≈ 0（多集 p50 +0.33/+0.73/−0.07）；
2. 录制规划轨迹近场横向 ≈ 0（p50 +0.18/+0.07）；
3. 本轮 corridor run 的 `line_lat`（定义 = **当帧全部检出标线横向的均值**，
   `fsd_drive._painted_line_lat`）与 GT 中心相关 +0.62、p50 −0.20 ≈ GT p50 −0.07
   ——**双侧线都被检出时 line_lat≈0 即居中**；`fsd_drive.py` 旧注释
   「+1.75 centred」是**只检出左侧单线**时的口径（山区常见），两套量此前被混用。

已改 `EGO_LANE_CENTRE_M = 0.0` 并留注释。**对 ROADMAP 的影响**：
- P1 的「in-lane ≥ 26%」门限是旧口径数值，新口径下不可比，需按新目标重定基线；
- 旧口径下 v13b/hand/v8 的 in-lane（24.3%/57.2%/…）全部不可比。新口径下同一批
  6 集（未钉名，会漂移）：v13b paired 23.8% / in-lane **96.6%**，hand 16.2% / 61.0%，
  v8 9.6% / 66.7% —— **v13b 两轴均优，"v13b 中心偏左"的旧结论疑似旧口径伪象**；
  但城镇 pin 的实车证据两会话相反（本会话 1v1：hand 9.5% vs v13b 30.5% 可用率；
  贵方 5v5+12 臂：hand 5/5 零违规、可用率反高），**pin 不动**，待钉住 episode 集
  + 每臂 ≥5 的交错 A/B 定夺。
- `tests/test_lane_corridor_center.py` 10 项、`pytest tests/` 全量、
  `m5_offline_validate` 均绿（旗标关 = 行为不变）。

## 22. 钉住集重跑定基线：城镇 pin 冲突解决——维持 hand（2026-09-12）

按测量纪律（钉住文件名 + 修正后目标）重跑：**钉住 10 集**
`logs/m5_seg/pinned_town_set.json`（101 个城镇影子集按名排序等间隔取 10，
跨 09-05 → 09-11、跨两会话；名单已固化），`--episode-names` + 修正后的
`EGO_LANE_CENTRE_M = 0`：

| 模型 | paired | 中心 lat p50 | in-lane |
| --- | --- | --- | --- |
| v13b | 22.2% (467 帧) | −0.366 | 83.1% |
| **hand（现 pin）** | **33.1% (696 帧)** | **+0.159** | **86.2%** |

**结论：hand 在两轴均优，城镇 pin 维持 hand 不变。** 证据三方一致：
钉住离线（本节）+ 贵方实车 5v5 与 12 臂（crossC 0/12、可用率更高）。
本会话此前支持 v13b 的两条证据均撤回，且都找到了测量根因：

1. **目标常数错误**（§21b 已记）：−1.75 口径系统性错罚 hand 一类
   "中心近 0"的模型；
2. **近因漂移集**：「最新 6 集」（全部来自 09-11 22:2x–22:4x 同一路段同一
   小时）给出 v13b 23.8%/96.6% vs hand 16.2%/61.0%，排名与钉住 10 集相反
   ——v13b 的 in-lane 96.6%→83.1%、hand 61.0%→86.2%。
   **§23 的测量陷阱（必须钉住 episode 文件名）再次被定量证实**，且本次
   受害者是我自己。

本会话 09-11 深夜那条「v13b 实车可用率 30.5% vs hand 9.5%」的 1v1，
与上述三方证据冲突，按单臂噪声处理（项目已知：同配置 off 计数 0–22 波动），
不作为 pin 依据。

**工具**：`m5_live_ab.py` 新增 `--seg-models A,B`（与 `--factor` 二选一），
可直接做「两个 checkpoint 同场景交错 A/B」（CLI `--seg-model` 覆盖场景
pin，故两臂只差模型）；如需对 pin 做 ≥5 臂/组的实车复核，用该入口并按
协议在本文档留 `vehicle free/taken`。

**ROADMAP P1 门限已据此更新**：钉住 10 集上 paired 显著 > 33.1% 且
in-lane ≥ 86%（hand 基线）；旧口径 "24.3% / 26%" 作废。

## 23. P1b 判别力验证通过：按侧 gap 规则零误判（2026-09-12，钉住 10 集，离线）

`scripts/m5_line_identity_probe.py`（新工具）：每帧取 GT 左/右线中位作为
"单边观测"，与可行驶栅格右边缘组成 (线横向 s, 路缘 R)，判别该线是本车道
右边界还是中线，与 GT 车道中心（左右线中点）比误差。钉住 10 集、3300 个
单线观测：

| 量 | 真右边界 (n=1650) | 真中线 (n=1650) |
| --- | --- | --- |
| gap = s − R 的 p10/p50/p90 | +0.31 / **+0.65** / +1.88 | +3.97 / **+4.35** / +4.94 |

**按侧规则（构造性零误判）**：右侧检出线只允许判「边界」或弃权（右行
双车道上中线不可能在车右侧）——`gap ≤ 2.5 m ⇒ 边界`；左侧检出线只允许
判「中线」或弃权——`gap ≥ 3.5 m ⇒ 中线`。右侧 gap 的宽路肩尾部（max
+7.56 m）与中线区间重叠，全局单阈值必有 ~4.4% 误判（=往对向偏 3.5 m），
按侧不对称消除它。

**最终工作点（lo=2.5 / hi=3.5）**：边界判定 93.4%（弃权 6.6%）、中线
99.9%；**隐含车道中心 vs GT：p50 误差 0.23 m，99.9% ≤1.2 m，91.3%
≤0.5 m**，总覆盖 96.6%。对照基线：旧单边镜像路径只有 ~10% 中心落本车道。

### 必须重看的历史结论

§18/§20 曾以"放宽镜像门让中心漂到 +0.16 m、车道内崩到 10.6%"否掉单边
路径——那是 **−1.75 旧口径**下的数字。修正口径（中心≈0）下 **+0.16 恰恰
接近完美居中**：单边路径被否决的直接依据疑似口径伪象（§21b 同源）。
本节规则等于给单边路径补上了它一直缺的"身份判别 + 路肩合理性校验"。

### 边界与本验证的适用范围

1. 观测线是 **GT annotation**，不是检测器输出——下一步必须把检测器的
   实际单边标记过同一规则在钉住集上重测（离线），再谈集成；
2. 单一路域（城镇 italy）；规则隐含"右行、本车在右车道"的结构先验，
   换路域需重验；
3. 集成（替换/放宽 `_mirror_right_ok` 一类门）= 行为改变，必须走完整
   实车安全门：line_lat 居中（居中读数 0 口径）+ 0 压线 / 0 出路面 /
   0 倒车，≥5 臂/组交错 A/B。

## 24. 检测器级重测：身份规则零误判保持，瓶颈转移到观测质量（2026-09-12）

`m5_line_identity_probe.py --source detector --model seg_model_hand`（当前
城镇 pin 的实际输出），钉住 10 集 2102 帧：

**帧人口**：paired 68.8%、单右边 6.6%、单左边 15.9%、无 marked 8.7%
（当前 hand+虚线归并下，单边路径可 addressed 的人口 ≈22.5%）。

| 量 | 右侧→边界 | 左侧→中线 |
| --- | --- | --- |
| 判定数 / 误判 | 127 / **0** | 209 / **0** |
| 隐含中心误差 p50 | **0.23 m** | 0.78 m |
| ≤1.2 m | 77.2% | 69.9% |
| 整体（336 判定） | p50 0.63 m，72.6% ≤1.2 m，7.4% >2 m | |

**结论**：
1. **§19 的悬置问题已回答**：按侧 gap 规则在检测器噪声下仍保持**零误判**
   （结构性保证：右侧检出线永不判"中线"）——灾难性失败模式（指向对向）消除；
2. **GT 级的 99.9% in-lane 不迁移**（预注册纪律兑现：离线达标≠检测器达标）；
3. **阈值扫描无效**（tb 1.5–2.5 / tc 3.5–4.5 全部 ~72–73%）→ 残余误差不在
   判别，在**单边观测本身的横向估计**（标记中位的中位）。右分支已接近
   配对路径的精度（0.23 m vs 配对 86.2% in-lane），左分支弱；
4. **集成门槛未达，不上车**。下一个离线杠杆是观测质量：点级均值聚合
   （替代中位的中位）已启动对照；以及标记质量门（跨度/点数）与时序中位。

**观测质量对照（点级均值 vs 中位的中位）**：p50 0.63→0.56 m、in-lane
72.6%→77.2%——小幅收益，仍低于集成门槛（~23% 判定帧 >1.2 m）。残余误差
更可能来自标记本身（碎片化/弯道弧段当作直线），聚合方式只是小头。
**下一步的离线问题因此转向**：检测器人口里 paired 标记（双侧都有标记）
已占 68.8%，而实车 lane 可用率只有 ~19-30%——从"双侧有标记"到"配对被
接受"之间 ~40 pt 损失落在融合/门控层（mirror 窗、锯齿分类、宽度/对齐）。
在钉住集 + hand 模型上跑 `m5_lane_continuity.py --diagnose` 定位这个漏斗
是下一条比单边路径更大的杠杆。

## 25. 配对漏斗逐门归因（钉住 10 集 + hand，修正口径）：候选门基本正确，几何门过严（2026-09-12）

`m5_lane_continuity.py --diagnose`（新增 `--episode-names` 钉住）+ 新工具
`m5_pair_gate_audit.py`（强制中点 vs GT）。2102 帧漏斗：

| 漏斗级 | 帧数 | 占比 |
| --- | --- | --- |
| paired（配对接受） | 696 | 33.1% |
| single_edge（门后单边） | 1158 | 55.1% |
| pair_failed（双侧都在仍拒） | 244 | 11.6% |

未配对原因（有标记帧）：`one_side_filtered_by_candidate_gate` **533**、
`both_sides_but_no_pair` **525**、`all_markings_filtered` 115、
`missing_right__extractor` 97、`pair_rejected_mirror_refused` **32**。

**逐门裁决（修正口径下重新评估）**：
1. **候选侧门拒得对**：`thin:side` 334 + `solid:side` 287 共 621 次丢弃，
   真漆占比 **0**——与 §11/§20 一致，不是损失源；
2. **镜像窗只剩 32 帧**——旧分析的主攻点已不是主要损失；
3. **几何配对门过严（主要可回收项）**：对 301 个 no_pair 帧强制取左右中点，
   中心质量与已接受的 532 帧**不可区分**（p50 0.79 vs 0.76 m，≤1.2 m
   85.4% vs 88.0%）。回收它们 ≈ +14 pt 帧的配对可用率；
4. 参照系提醒：**已接受配对的 module 中心对 GT 也只有 76.8% ≤1.2 m
   （p50 0.62 m）**——"配对=正确"本身也只有 ~77%，与 §22 的 86%（对目标 0）
   是两个口径，都记录在案。

**下一步（离线）**：定位 `pair_lane_markings` 内部拒绝这 301 帧的具体判据
（宽度/对齐/中心偏移），在钉住集上做受控放宽 + 用本审计的中心质量当验收，
达标后走实车安全门。small bucket（mirror_refused 9 帧，44% 合格）不值得动。

## 26. 几何门放宽实验：否决——「配对门过严」假设不成立（2026-09-12，钉住 10 集）

`pairing._best_vision_pair` 已加**只读拒绝计数**（`debug["pair_rejects"]`/
`debug["axis_rejects"]`，18 行插桩，pytest 全绿）。no_pair 帧的拒绝直方图
（466 帧）：`one_near_far_start` 314、`far_kind` 239、`width` 165、
`far_width` 145、`near_combo` 65、`center_prefer_width` 45。

**受控实验**（`m5_pair_gate_audit.py --relax`，monkeypatch 常量，库零改动）：

| 实验 | 放宽 | 接受(GT打分) | 回收帧质量 (module 中心 vs GT) |
| --- | --- | --- | --- |
| 基线 | — | 532（p50 0.62，76.8% ≤1.2m） | — |
| A | far-start 8→15 | 537（**只 +5**） | — |
| B | A + far-width 4.5→3.0 + width-max 6.5→8.5 | 559（+27） | **30 帧 p50 0.93 m，63.3% ≤1.2 m**；另丢 3 帧 |

**验收线（p50 ≤0.8 m 且 ≥85% ≤1.2 m）：不达标 → 不集成。**

**教训（为什么 §25 的乐观是错的）**：§25 的"强制中点 85.4% 合格"用的是
"全部候选近场中位的中位"——它平均了候选；模块实际打分选中的具体配对在
这些帧上质量差得多（63%）。**"存在一个好中点"≠"模块能选出一个好配对"**，
验收必须用模块真实输出。

**今日总账（配对漏斗线）**：候选侧门正确（0 真漆）；镜像窗 32 帧非主损；
几何门放宽被否决。剩余损失（single_edge 55%、no_pair ~20%）的根因在
**检测/标记质量轴**（碎片化标记、unknown 类、弯道弧段），不在门控阈值。
可走的路只剩：P1a 人工标注（需你）、P1b 检测轴深耕（含近场侧向相机——
3.59 m 盲区是结构性缺口）、以及 P6 权重可复现。
vehicle taken 01:5x (pillar probe: m5_pillar_capture.py, 40 stations town route, three annotation cams front+pillar L/R; will write 'vehicle free' right after)
vehicle free 12:55 (pillar capture done: logs/m5_pillar/pillar_20260912_124924.npz, 40 stations)

## 27. P1b 侧向相机：采集管线就绪，但撞上「teleport 后漆画 decal 缺失」的世界状态问题（2026-09-12 凌晨）

新工具 `scripts/m5_pillar_capture.py`（capture + --analyze）：三路 annotation
相机（front_main / pillar_left / pillar_right，536×403），站位取影子集真实
路上位姿。机制上跑通了（40 站 × 三视角，`logs/m5_pillar/pillar_*.npz`），
**但测量无效**，原因已定位到世界状态：

* 影子集在位姿 (779.26, 733.74) 行驶采集时有 **5343 GT 线像素**；
  我的 teleport 采集在同一位姿只有 **~3 px**——RGB 本体也缺中央虚线
  （对比 `_dbg_front_*.png` vs 影子集 RGB 可见），不是标注色表或相机几何问题；
* 已排除：相机参数（用 collect_seg 原常量 A/B 同结果）、流式加载延迟
  （等待 12 s 无变化）、标注色映射（原始 annotation 里根本没有线类颜色）；
* 裸 `conn.load_scenario()` 挂起（已杀）；工作管线的差异假设：fsd_drive/
  collect_seg 的世界是**行驶/完整场景状态**，漆画 decal 已生成。
* **测量完整性陷阱（记档）**：本项目里 teleport 静态采集会静默缺路面漆画，
  任何依赖 decal 的离线采集都必须在**行驶中**做（collect_seg 正是因此无恙）。

**下一步**：把 pillar 探针改成行驶采集（沿城镇路线 AI 驾驶、连续采三视角，
复用 `m5_lane_truth_probe --drive` / collect_seg 的驱动模式），或复刻
fsd_drive 的场景初始化后再 teleport。采集有效后跑既有 --analyze 量化盲区增益。
vehicle taken 13:2x (pillar drive-capture: m5_pillar_capture.py --frames 90, AI span; will write 'vehicle free' after)

**用车协调事件（13:5x）**：发现两个 `m5_pillar_capture.py --frames 90` 实例
同时运行（venv python 与系统 python 各一，后者非本会话所起，疑似并行会话
读了本节后接着执行同一动作）。单车/单游戏下双实例必然互卡，两轮结果均作废，
已按 PID 双杀。**谁要跑 pillar capture，先在本文件写 taken 并检查进程表**。
vehicle taken again 14:0x (single clean pillar drive-capture, unbuffered log)

**用车协调事件 2（14:1x）**：pillar drive-capture 再次双实例（venv 8532 +
系统 python 25444，后者非本会话所起）。同时观察到 `m5_line_grid_labeler`
在跑——**并行会话正在做 P1a 人工标注**。本会话已杀掉自己的实例 8532 并
**释放车辆**给 P1a；drive-capture 代码就绪，等干净窗口再跑。注意：
同一命令出现"venv + 系统 python"成对双起的模式已两次，原因不明（疑似
某个会话的启动方式差异），跑长任务前先查进程表。
vehicle free 14:1x (pillar drive-capture deferred; P1a labeling has priority)

**状态收尾（14:3x）**：本会话已停止自己的 pillar capture（8532）并释放
车辆给 P1a 标注。**遗留**：本会话被杀时 AI span 模式没走清理路径，随后
的"attach + ai disable"也挂起（游戏连接被多会话争用）——**车可能仍在
AI 慢速行驶**。任何要用车/附着车辆的会话请先 `ai.set_mode("disabled")`。
drive-capture 代码就绪（`m5_pillar_capture.py`，已验证机械可跑），有效
窗口 = 进程表只有一套 pillar/labeler 进程时。
vehicle taken 14:5x (clean window: disable leftover AI then pillar drive-capture 90 frames)
