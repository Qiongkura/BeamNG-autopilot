...
实验工具的四处量错（T12）。`pytest tests/ -o addopts='' -q` → **2068 passed**（本轮开始前 2018）。
...
**实车**（`logs/goal_20260921/t09_risk_live{,2}.json`，各 36 帧）：
- `lateral_risk` 36/36 帧，但**该路段没有发布双侧边界** ⇒ `gap_left/gap_right/first_crossing` 全为
  **UNKNOWN** 并给出原因（"boundary not published"）——计划要求的"缺边界输出 UNKNOWN"是**实车跑出来的**，
  不是断言出来的；同时也说明这条路上的横向风险**今天根本算不出来**，得先有双边边界。
- `hold_audit` 36/36 帧，`enforced=False`（门默认关，行为未变）；`satisfied` 7–8 帧，其余为
  `unknown: sigma_lat/sigma_theta`（影子状态尚未测量）与 `failed: remaining path unknown`（该 tick 没有可用路径）。
  首版把"尚未测量"的初值协方差（1.0）当成"measured 且超限"来失败——已修：未测量 → `unknown`。
...
真实连续片段上的完整标注与质量统计未做；**金帧复制/相邻片段检测未做**；旧 `logs/` 采集没有新字段，
训练入口对其如实打印回退提示（新字段只对之后的采集生效）。
...
| 阶段 2–6（四臂重跑、场景矩阵、影子能力） | T06（§6d）、T07 第一版（§6e）、T08（§6f）、T09（§6g）、T11 第一版（§6h）已落地；**阶段 C 第一项已完成**（§6i，每臂 8 次交错，结论=不采纳）；阶段 C 余项（更长的对照路线、独立真值的压线/出铺装）已完成可做部分；**T07/T10 余项本轮补齐可做部分**（§6k、§6l）；阶段 D 未开始 |
