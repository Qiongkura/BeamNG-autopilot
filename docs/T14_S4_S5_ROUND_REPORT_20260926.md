...
| D2 学习循环可解释 | **部分** | S4 看板（七面板 + 页面实检）、S5 因子有效性与单因归因已落地并有测试；缺"因子真实激活"的真实训练轮与受控 S6 结果 |
| R2 离线候选 | 未开始 | 身份率 0.3115/0.3280 远低于 0.60，无可晋级候选；最终集未消费 |
| R3 Tech 候选 | 未开始 | 依赖 R2 |

* **候选去留**：两个权重都不满足身份门（0.60），不晋级；也不降低门槛。
* **下一轮输入**：S6 定向实验（E1 困难负例组成 / E2 路口弯道漏检）。**前置约束**：
  线通道的可晋级训练标签目前只有人工复核包（= 开发/评价集，不得用于训练），
  所以 S6 若用线通道只能以研究臂运行（判定记 `research_only`，不允许晋级）；
  要做可晋级的线通道实验，需要新的 verified 线标签训练数据（用户侧标注）。
* **停止理由**：无（未触发停止条件）。
