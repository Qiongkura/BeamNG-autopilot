...
真正剩下的两个缺口：① 长路段上本车道**另一侧边界根本检不出**；
② 离线 strict 能接受单边镜像帧（45%），线上融合只给 11%——差口在
`choose_sensor_lane` 那几道**依赖 LiDAR 帧与跨帧 hold 状态**的守卫
（`_vision_mirror_keeps_reference` / `_vision_edge_inside_lidar`），
而影子集**不记录 `ray_hits`**，所以这条路目前无法离线复现。
下一步要么让录制包含 LiDAR 与融合结果（使融合路径可离线回放），要么改长距标线检出。
