...
用正确模型后，瓶颈落在**掩码下游**：217 帧里产出 109 条标线，但分类结果只有
19 条 `solid`，而 `thin` 46 条、`unknown` 41 条；`_collect_candidates` 丢掉
**105 条 `unknown:kind`**；提取器另有 **1279 个连通域被丢**，其中 `small_h`
961 个（`w_p50=17, h_p50=5, area_p50=72` 的短宽块）。

**以下四条"已排除"的修法都是在错误 checkpoint 下测的，需要重新验证后才能引用：**

1. ~~放开标线提取守卫~~（`_mask_to_markings` 的 `min_height`/填充率/包围盒）：
   当时测到标线数 3.0→57.9/帧、配成率不动。**结论作废**，需用 v13b 重测。
2. ~~放开候选门控~~（`_collect_candidates` 的侧向 5 m / 跨度 / 对齐）：
   当时测到被丢标线侧向中位数 9.4/7.0/−5.9 m。**结论作废**，需用 v13b 重测
   （v13b 下的主要丢弃项已变为 `unknown:kind` 105 条）。
3. 怪分割模型：**已判定不成立**——用对模型后召回 73–81% 且左右对称。
4. 时序延续（last-good hold）：掉线是**长时缺失**而非抖动——最新一轮 16 段、
   185 帧、共 101.6 秒，**最长一段 35.6 秒**，1 秒以内的缺口总计只有 0.3 秒。
   这一条与 checkpoint 无关（时长统计），结论保留。

另有一条与 checkpoint 无关、仍然成立的结论：`lane_frame_usable` 的
`LANE_PAIRED_VISION_MIN_SPAN_M = 6.0` 门槛**不能降**——3.2 m 的短配对可以在
"每个点都离车很近"的同时让中心偏离导航路线好几米，正是 runs 42/47/52 把车拽偏
的原因（`m5_offline_validate.py` 的 `lane-guard` 检查）。本轮实测过降低它的后果：
放行帧中位中心落到 −1.44 m（车道内 82%），但**反面就是 18% 不在车道内**——聚合
指标不能否证一条为尾部风险存在的守卫，故已撤回。该门现已补上缺失的单元测试
（`tests/test_lane_fusion.py`）。

下一步：在 v13b 下重做提取器与候选门控的归因，重点看那 105 条
`unknown:kind` 与 961 个 `small_h` 里有多少是真标线。
