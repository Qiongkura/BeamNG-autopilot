...
- 车道侧与录制（2026-09-02）：`m5_fsd_drive` 起步/重启落**右车道**（不再从路线
  中线起步）；录制（`m5_shadow_drive`）用 map_lane 右车道参考 + lane_center
  候选，标签不再骑中线；`fsd_drive` 新增在线**漆画线横向指标**（`line_lat`，
  每帧投影语义标线报车在标线哪侧，结束汇总）。`m5_autopilot --fsd` 可把 FSD
  分层规划作为转向前端（规则路径兜底，默认关闭）。
