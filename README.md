...
### 11) 优化（2026-08-28，opt24 代码级修复，待实车验证）

`git a2ae0ea..c4cdad0` 三个独立提交（每项一个），均为纯代码改动 +
离线回归（pytest 327 项全过、`m5_offline_validate.py` ALL PASS），
**未做实车验证**（用户要求先不做实车测试）：

| 提交 | 修复 | 做法 |
| --- | --- | --- |
| `66ee47a` | **FSD tick 卡顿**（opt23：range p90 283ms、77/186 帧 >300ms，实车每几帧一卡） | LiDAR 扫描范围 120→80m、障碍聚类半径 40→30m（管线实际只用 55m / 25m）；`FSDStack` 每 3 tick 轮询一次 LiDAR，中间复用上次世界系扫描——静态墙仍有效，时序占用滤波桥接间隔 |
| `233d6ca` | **路口几何误读蠕行**（opt23 t=36-45 连续 11 帧 plan=1.97 m/s，路口 5s+ 爬行） | 弯前 governor 不再看局部 `route_rad<15m`，改为 12m 前瞻内真实累计转向 `_route_turn_deg` ≥40° 才限速（真发卡弯 60-180°；路口圆角 R~3m 但总转向 <30° 不再蠕行） |
| `c4cdad0` | **起步/重启油门冲击**（opt23：13 帧速度跳变 >1.5 m/s、throttle 翻转 51） | 新增 `rate_limit_pedal()`，所有 override（下坡 cap / overspeed taper / bend governor）之后对最终踏板限幅（0.8-1.2/s 线性斜坡）；硬停/爬坡/倒车/终点抱死等安全分支绕过 |

注意：LiDAR 隔帧复用暂未做运动补偿，静态墙安全，动态障碍最多延迟
~3 tick 刷新——实车验证时重点观察路口/来车。
...
| **路口几何误读蠕行** | 路口圆角局部半径 R~3m 触发弯前 governor，plan 锁 1.97 m/s 爬行 5s+ | 高 | ✅ 已解决（governor 改按 12m 前瞻真实累计转向 ≥40° 门控，`_route_turn_deg` 按段航向差求和，opt24 代码级 + 单测，待实车确认） |
| **起步/重启油门冲击** | 重规划后 0→0.8 油门一步给满，速度每帧跳 >1.5 m/s | 高 | ✅ 已解决（`rate_limit_pedal` 对最终踏板限幅 0.8-1.2/s，安全分支绕过，opt24 代码级 + 单测，待实车确认） |
...
| **FSD tick 慢** | 感知规划 tick 曾 1.4~7s，实车一顿一顿 | 中 | ✅ 已缓解（实时模式 ~2Hz 连续控制、无暂停卡顿；warm-up + 只抓前视 + LiDAR 每 3 tick 轮询 + 扫描范围 120→80m / 聚类半径 40→30m + 向量化 + 密度 12→8 / 点数 6000→4500，opt23 p90 283ms → opt24 代码级再降，待实车确认） |
| **长测未完成** | 完整路线终点、城镇十字路口、密集 LiDAR 场景尚未长测完 | 中 | 已跑 opt14（30s / 290m）与 opt23（95s / 284m 完整路线：0 压线、0 倒车、0 上草、终点 2.85m）；城镇路口多轮长测待做 |
