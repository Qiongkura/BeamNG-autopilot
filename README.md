...
### 感知车道连续性：已量化的瓶颈与被排除的修法（2026-09-10）
...
离线复现工具 `scripts/m5_lane_continuity.py` 用已录影子集跑同一条感知路径
（前视相机 → SemanticHead → 世界标线 → `pair_lane_markings`），在 3 集 654 帧上
量到的漏斗是：

| 环节 | 数值 |
| --- | --- |
| 配成双侧本车道 | 32.3%（线上融合后 11%） |
| 未配对帧：掩码有漆但没产出标线 | 约 38%（右侧为主） |
| 未配对帧：标线产出但被候选门控丢掉 | 约 29% |
| 未配对帧：两侧都有候选但配不出对 | 约 20% |
| 未配对帧：模型真漏检（GT 有漆、掩码没有） | 约 7% |

**以下四条修法已用测量排除，不要再走：**

1. **放开标线提取守卫**（`_mask_to_markings` 的 `min_height`/填充率/包围盒）：
   标线数从 3.0/帧暴涨到 57.9/帧（真实道路只有 2–4 条），配成率**完全不动**
   （33.6%）；只为放进噪声就把配成率"提"到 41.9%，配对中心同时从 −1.386 m
   漂到 −1.17 m。守卫是承重的。
2. **放开候选门控**（`_collect_candidates` 的侧向 5 m / 跨度 / 对齐）：
   被丢的标线侧向中位数是 9.4 m / 7.0 m / −5.9 m，那是别的车道和路口横穿线，
   丢得正确。
3. **怪分割模型**：GT 有漆而掩码没有的只有 9/217 帧（约 7%）。
4. **时序延续（last-good hold）**：掉线是**长时缺失**而非抖动——最新一轮 12 段、
   144 帧、共 79.1 秒，**最长一段 20.9 秒**，1 秒以内的缺口总计只有 1.0 秒。
   真实 FSD 那种 1–2 s hold 只能救回几秒。

两个剩下的缺口，差口②已量化，①已定位到具体形态：

① **左右检出严重不对称**：配对帧里每一帧都是同一个模式——左侧边界是实线、
稳定检出 24–28 m；右侧边界只检出一段 2–3 m 的 `dashed` 碎片。配对跨度取
两者的纵向交集，于是等于右线那 2 m。而且每帧候选总数只有 2 个
（`cands n=2`），右侧**没有其它碎片可合并**——那一段之外的右线根本不在掩码里。
所以真正的修法是**让右侧边界被检出得更长**（长距/弱标线检出，属分割与数据侧），
不是配对或门控的问题。

② 离线 strict 能接受单边镜像帧（45%），线上融合只给 11%。复盘 217 帧后
差口定位在 `choose_sensor_lane` 的 `lane_frame_usable` 门：感知前端其实
**87.6% 的帧都产出了 lane frame**（190/217），但 `lane_frame_usable` 只放行
18%——其中 **73 个配对帧一个不剩**，全部卡在 `span_m < 6.0`；线上那 24 个
`lane_paired` 帧其实来自 LiDAR 走廊，**视觉配对车道线上从未被采用过**。

而 6.0 这个门槛**是有正当理由的、不能降**：`m5_offline_validate.py` 的
`lane-guard` 检查记载，3.2 m 的短配对可以在"每个点都离车很近"的同时让中心
**偏离导航路线好几米**，正是 runs 42/47/52 把车拽偏的原因。本轮实测过降低它的
后果：放行帧的中位中心会落到 −1.44 m（车道内 82%），但**反面就是 18% 不在车道内**，
即那条尾部风险——聚合指标不能作为否证尾部守卫的依据，故已撤回。

结论：**视觉配对的跨度本身才是门槛**。配对跨度中位数只有 2.6–2.8 m，
所以真正的修法是让**被检测到的线更长**（长距标线检出），而不是放松这道门。
如果之后要动融合路径，再让录制包含 `ray_hits` 与融合结果使其可离线回放。
