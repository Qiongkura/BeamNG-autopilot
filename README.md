...
### 8) 最近实车验证（2026-08-27，fix56 长测）

`logs/fix56.json`：75s / 20 帧 FSD 实车（Tech，italy 山地发卡弯路线，
起点 (729,763) → 终点前 (768,737)）：

| 指标 | 结果 |
| --- | --- |
| 安全级别 | 全程 `safe`，0 次 minimal_risk / emergency |
| 倒车 / 卡死 | 0 次 reversing / stuck |
| 车道内横向偏移 | `lat_left` -1.80 ~ +1.00 m（未压中线、未逆行、未上草） |
| 第一发卡弯 | 干净通过，入弯前即打方向，不再冲过弯心外侧 |

修复根因：第一弯入弯偏晚 = **控制时序 bug**（仿真在 FSD tick 计算期间
实时跑，首帧感知+规划约 7s，车先直滚 ~9m 错过发卡弯）+ 首帧转向 dt=0，
由确定性控制时序 + 首帧转向解决；坡道倒车滚回由 `ReverseManeuver` 有界
倒车 + R 挡油门只在速度回正时给解决。

### 9) 最近实车验证（2026-08-28，opt14）

`logs/fsd_drive_verify_opt14.json`：30s / 58 帧 FSD 实车（Tech，italy
road-graph A* 路线 290m，起点 (729,763) → 目标 (572,533)）：

| 指标 | 结果 |
| --- | --- |
| 安全级别 | 全程 `safe`，0 次 emergency / minimal_risk / stuck |
| 倒车 / 停车 | 0 次 reversing、0 次 force-stop |
| 速度 | 3.8~6.9 m/s（目标 6），无急刹、无爬行 |
| 车道 | 全程在右车道内（横向 1.3~1.9 m 于路线右侧），未压中线、未上草 |
| 传感器车道 | 方向与导航一致，无假弯误检 |

本轮同时修复：传感器车道 heading gate 60°→35°（拒掉假左弯）、车头偏航
自动降速（消除 7~8 m/s 转向振荡）、地图道路边缘护栏（越界降速，禁止上
草地）、无路线限速 1 m/s、地面安全重生（任何图不会重生在地下）。
...
### 10) 最近实车验证（2026-08-28，opt23 完整路线 + 一键评估）

`logs/fsd_eval_verify_opt23.json`：95s / 186 帧 FSD 实车（Tech，italy
road-graph A* 路线 284m，起点 (729,763) → 目标 (572,533.5)），由新增的
`m5_fsd_eval.py --runs 1` 一键跑完并自动出报告：

| 指标 | 结果 |
| --- | --- |
| 安全级别 | 全程 `safe`，0 次 emergency / minimal_risk / force-stop |
| 压线 / 逆行 / 上草 | 0 次 cross_centre、0 次 cross_right、0 次 off_road |
| 倒车 / 卡死 | 0 次 reversing；stall 9 → 4 帧（仅起步暖机与两处瞬停，均自动恢复） |
| 振荡 | throttle 翻转 53→51、brake 翻转 37→35、峰值速度 7.42→6.72 m/s、蠕行占比 36.2%→31.2% |
| 终点 | 停在车道内，距目标 2.85 m，`rem_end` 5.42 m |

本轮修复：**路口/终点 plan 速度 1.0↔6.0 逐帧跳变**（LiDAR 杂波随帧
出现/消失 → 刹车 0.75 死停 → 全油门重启，opt22 t≈52 路口卡死 ~3s）——
`plan_speed` 限幅（下 2 m/s² / 上 1 m/s²）+ 弧长投影采样（替代最近顶点，
消除索引抖动）+ 走廊开放时速度剖面的障碍下限提到 0.4×巡航（与安全监控
同规则）。

### 11) 实车验证（2026-08-28，opt24 三修复 + 完整路线复测）

`logs/fsd_eval_verify_opt24.json`：95s / 186 帧 FSD 实车（Tech，italy
同一路线 284m，起点 (729,763) → 目标 (572,533.5)），与 opt23 同条件
对比：

| 指标 | opt23 | opt24 |
| --- | --- | --- |
| 安全级别 | 全程 safe | 全程 safe，0 emergency / minimal_risk |
| 压线 / 逆行 / 上草 | 0 / 0 / 0 | 0 / 0 / 0 |
| 倒车 / 卡死 | 0 次 reversing、stall 4 | 0 次 reversing、stall 1（仅起步暖机） |
| 速度跳变 >1.5 m/s | 13 帧 | **4 帧**（全部为发卡弯/终点减速，无起步油门冲击） |
| throttle / brake 翻转 | 51 / 35 | **43 / 27** |
| FSD tick（total p90 / >300ms 帧数） | 447ms / 77 帧 | **364ms / 42 帧** |
| LiDAR range p90 | 283ms | **246ms** |
| 路口蠕行（plan≤2.1 连续≥3 帧） | t=36-45 连续 11 帧 | **无**（终点抱死段除外） |
| 终点 | 距目标 2.85m | 距目标 2.69m，停在车道内 |

三个提交（每项一个，`git a2ae0ea..c4cdad0`）：
- `66ee47a` LiDAR 扫描范围 120→80m、聚类半径 40→30m、`FSDStack` 每
  3 tick 轮询一次（中间复用上次世界系扫描，时序占用滤波桥接）。
- `233d6ca` 弯前 governor 改按 12m 前瞻真实累计转向 `_route_turn_deg`
  ≥40° 门控（真发卡弯 60-180°；路口圆角 R~3m 但总转向 <30° 不再蠕行）。
- `c4cdad0` `rate_limit_pedal()` 对最终踏板限幅（0.8-1.2/s），安全分支
  （硬停/爬坡/倒车/终点抱死）绕过。

已知注意：LiDAR 隔帧复用暂无运动补偿，静态墙安全；动态障碍（来车）
最多延迟 ~3 tick 刷新，城镇路口长测待下一轮重点观察。

### 12) 实车验证（2026-08-28，opt26 城镇路口 + 山地回归）

城镇路线（Tech，italy 城镇连续道路，起点 (729.6,763.9) → 目标
(832.55,731.59)，road-graph A* 118.6m / 89 点，全程 road-edge 完整），
`logs/fsd_town_run12.json`：60s 内到终点并停在车道内——

| 指标 | 结果 |
| --- | --- |
| 安全级别 | 全程 safe，0 emergency / minimal_risk / 卡死 |
| 压线 / 逆行 / 上草 | 0 / 0 / 0（`road_off` 全程 0，`reversing` 0） |
| 横向包络 | 中线右偏最大 +1.49m（修复前 +5.25m 出右缘）；路口 115° 转弯弯心短暂左偏 -2.71m（弯心几何固有，随即回正） |
| 终点 | 0 stops，停在 end-stop 区，车道内 |

山地回归（同一 284m 路线，`logs/fsd_eval_opt26.json`）：95s / 180 帧，
0 压线 / 0 逆行 / 0 上草 / 0 倒车，travel 284.1m，终点距目标 2.65m。

本轮三个提交（每项一个，`git ea8aaba..f467716`）：
- `882578b` 离路硬停：上草不再 0.5m/s 蠕行，改为全刹 + 手刹（opt24 前
  车切弯后在草地蠕行 20m 的直接修复）。
- `7b5a475` 离路检测改「导航中线 + 局部道路半宽」：DecalRoad 边界在
  路口拼接处会折叠（k=6 两条路的 edge 跳 11m），`_boundary_lateral`
  会吸到远处折角，把直线段正常路面误判为离路 3.2m 硬停（run10
  t=11）；中线法对折角免疫。
- `f467716` 路口转向门控：导航路线 12m 前瞻累计转向 ≥25° 时拒绝
  vision/LiDAR 车道（改用地图先验车道）——修复前传感器车道在路口读
  偏 24°（-48.6° vs 路线 -24.6°，35° 航向门放行），车 S 形先左后右
  （中线左 2.7m → 右缘外 5.2m）；现城镇全程 map_lane，直线段传感器
  车道仍可用。

已知注意：城镇路线目标必须是**路上的点**——旧目标 (799.8,768.6) 不在
road graph 上，A* 尾段是跨草地的直线（车沿尾段上草是路线问题而非控
制问题）；当前用路上的节点目标。

### 13) 实车验证（2026-08-28，opt32 终点停车回正）

`logs/fsd_eval_opt32.json`：95s / 186 帧 FSD 实车（Tech，italy 同一 284m
路线），0 压线 / 0 逆行 / 0 上草 / 0 倒车，travel 284.0m，终点距目标
2.7m，**停车位置中线右 1.77m（车身完全在车道内，修复前 0.73-0.88m 车身
跨线）**。

根因：目标 (572,533.5) 不在 road graph 上，A* 到最近节点后直插目标，
路线尾段折返约 1.8m；折返处 DecalRoad 左右边界互换，`map_lc_smooth`
（几何车道中心）退化成中线附近（0.15-0.77m），end-stop 参考把车拉向
中线停车。

修复（`m5_fsd_drive.py`）：
- 几何车道中心只采用「右边界真正在路线右侧」的行（发卡弯顶点/路尽头
  边界互换行用中位半宽平行偏移替代）。
- end-stop 参考方向只用**车前方最近的有效车道中心点**（远处折返退化点
  不再把方向带偏）。
- end-stop 参考从「沿车道方向的直线」改为**指向车前方 1.8-3.5m 的右侧
  车道中心点**：直线参考没有横向牵引（car-anchored 车道中心让车保持入
  场偏移），指向目标点后 PurePursuit 右拉 16-25°，车在减速段从右
  0.94m 收敛到右 1.77m。

### 14) 实车验证（2026-08-28，opt35 终点车头回正）

`logs/fsd_eval_opt35.json`：95s / 186 帧 FSD 实车（Tech，italy 同一 284m
路线），0 压线 / 0 逆行 / 0 上草 / 0 倒车，终点距目标 2.69m，停车
位置中线右 1.44m（不压线），**停车车头 -136.0°（车道方向约 -133.5°，
偏差 ~2.5°；修复前 -146°，偏 13°）**。

上一版「指向车前车道中心点」的横向牵引把车拉回车道，但车头在减速段
被带偏，停住后还继续满舵打方向（opt32/33）。本轮：
- 横向牵引只在 v≥1.5 m/s 时启用（指向车前 3-6m 右侧车道中心点），
  v<1.5 后参考改回「从车位置沿车道方向」的直线，低速段车头回正。
- 终点停区 v<0.5 时方向盘归零，不再停车后继续打方向。

### 15) 实车验证（2026-08-28，opt42 终点居中 + 车头回正）

`logs/fsd_eval_opt42.json`：95s / 177 帧 FSD 实车（Tech，italy 同一 284m
路线），0 压线 / 0 逆行 / 0 上草 / 0 倒车，travel 284.2m，终点距目标
3.37m。**停车位置中线右 2.28m（车道中央，不再贴中线），车头 -134.1°
（车道方向 -133.5°，偏差 0.6°）**。

问题链：目标 (572,533.5) 不在路上 → 路线尾段折返 → 终点最后 ~10m 的
中线/边界/方向数据被污染 → end-stop 参考偏左，车停在中线右 1.0-1.7m
且车头歪（opt35-41）。本轮修复：
- `_trim_backtrack`：裁剪路线末端折返尾段，end-stop 参考只使用真实道路
  部分（road-graph 中线 + 右 2.0m = 车道中心）。
- 横向牵引从 rem 20m 就开始（`END_PULL_START_M=20`，刹车仍在 12m 起），
  车有足够距离从右 1.0-1.4m 收敛到车道中央。
- v<1.0 后参考回到沿车道方向直线，车头回正；停住后方向盘归零。

### 16) 车道参考策略 lane-mode（2026-08-28，map / auto / sensor 实测对比）

新增 `--lane-mode`（`m5_fsd_drive` / `m5_fsd_eval` 均支持），三档：

| 模式 | 逻辑 | 实测（同一 284m 山地路线） |
| --- | --- | --- |
| `map`（默认） | 地图车道主导，感知车道通过严格三关（航向/转弯/侧向）才接管 | 0 压线 / 0 上草 / 0 倒车，284m 完整到终点（opt44） |
| `auto` | 感知车道仅在「与地图车道近端一致（≤1.5m）」时接管，边界始终用地图硬护栏 | sensor 全被门控拒绝 → 实际等价 map（opt43 环境偶发乱跑，重跑 map 正常） |
| `sensor` | 感知主导：关转弯门控、侧向放宽，地图中线/右缘仍为硬护栏 + 安全监控兜底 | 感知车道大量接管，但**压线 18 次、上草 14 帧、车道偏离 2-6m**（opt45）；0 emergency，护栏有效但感知质量不达标 |

结论：**当前 vision/LiDAR 车道（pair_lane_markings / build_lidar_corridor）质量不足以做 FSD 车道参考**——这是「感知优先」落地的真正瓶颈，不是 lane 融合结构。护栏（地图中线禁跨、右缘禁出、离路硬停、安全监控）在三档下都成功兜底。下一步应提升感知车道置信度/质量（配对门控、corridor 参数、视觉车道线），而不是继续放宽 lane 融合。
...
| **长测未完成** | 完整路线终点、城镇十字路口、密集 LiDAR 场景尚未长测完 | 中 | opt26 已补：山地 284m 完整路线（0 压线 / 0 倒车 / 0 上草，终点 2.65m）+ 城镇 118.6m 连续道路（0 压线 / 0 上草 / 0 倒车，到终点）；多场景长测继续 |
