...
| **速度节流轻微振荡** | 加速→轻刹→再加速，不影响安全但观感差 | 中 | ✅ 已缓解（目标速度平滑 + SpeedController 滞回 + 下坡起步油门门控，fix66 实车 0 刹停、油门平滑） |
| **FSD tick 慢（每帧 3~7s 墙钟）** | 0.75s 仿真突发，实车一顿一顿 | 中 | ✅ 已缓解（相机环 warm-up 一次 + 实车只抓前视 + LiDAR 隔帧 + 规划/安全距离向量化，感知规划 tick 降到 0.4~0.7s，帧间隔 3.2s→2.7s；剩余大头是 BeamNG 物理 step 本身 ~50ms/步） |
| **长测未完成** | 完整 117m 路线终点、城镇十字路口、密集 LiDAR 场景尚未长测完 | 中 | fix56 仅验证了首段发卡弯；已有走廊连通门控 + 仲裁兜底 |
